...
* **Why Initial F1-scores are High:**
    As observed in Fig 1, both training and validation F1-scores start at a relatively high level...
    1.  **Balanced Sampling:** ...
    2.  **Distinct Visual Features:** ...
    3.  **Dynamic Thresholding and Inference Strategy:** During our local training and evaluation, we searched for the optimal threshold (from 0.1 to 0.95) to maximize the F1-score and analyze the model's full potential. However, for the final submission script (`For_TA_test.py`), since ground truth labels are unavailable in the test environment, we deployed the model with a **fixed threshold of 0.5**. This ensures the inference process is fully compliant with the requirements (generating a JSON prediction file without accessing labels).
